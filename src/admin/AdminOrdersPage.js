export class AdminOrdersPage {
        ...
        <div class="admin-content">
          ...
          <div style="background: white; border-radius: var(--border-radius); box-shadow: var(--shadow-sm); overflow: hidden;">
            <div style="padding: 1.5rem; border-bottom: 1px solid var(--border-color);">
              <h2 style="margin: 0;">Lista de Pedidos (${
                  ...
                  <tbody>
                    ${this.filteredOrders
                      ...
                      <tr style="border-bottom: 1px solid var(--border-color);">
                        ...
                        <td style="padding: 1rem;">
                          <div>${new Date(order.createdAt).toLocaleDateString(
                            "es-ES"
                          )}</div>
                          <div style="color: var(--text-secondary); font-size: 0.875rem;">${new Date(
                            order.createdAt
                          ).toLocaleTimeString("es-ES")}</div>
                        </td>
                        <td style="padding: 1rem;">
                        ...
                        <td style="padding: 1rem;">
                          <span class="status-badge status-${this.getStatusClass(
                            order.status
                          )}">
                        ...
                        <td style="padding: 1rem; text-align: center;">
                          <div style="display: flex; gap: 0.5rem; justify-content: center;">
                            <button class="btn btn-sm btn-outline" onclick="this.viewOrder('${
                              order.id
                            }')">👁️ Ver</button>
                            <button class="btn btn-sm btn-primary" onclick="this.updateStatus('${
                              order.id
                            }')">📝 Estado</button>
                            ${
      ...
      order.customer.lastName
        ...
        <div style="margin-bottom: 2rem;">
          ...
          <div style="background: var(--bg-secondary); padding: 1rem; border-radius: var(--border-radius); margin-top: 1rem;">
            ...
            <div>${order.customer.city}, ${order.customer.state} ${
            ...
            ${order.items
              ...
              <div style="display: flex; justify-content: space-between; align-items: center; padding: 1rem; border: 1px solid var(--border-color); border-radius: var(--border-radius); margin-bottom: 0.5rem;">
                ...
                <div style="text-align: right;">
                  ...
                  <div style="font-weight: 600; color: var(--primary-color);">$${item.price.toFixed(
                    2
                  )}</div>
                </div>
              </div>
  ...
  updateStatus(orderId) {
    ...
    const content = `
      <div>
        <p style="margin-bottom: 1rem;">Estado actual: <strong>${this.getStatusText(
          order.status
        )}</strong></p>
        ...
        <select id="new-status" style="width: 100%; padding: 0.75rem; border: 1px solid var(--border-color); border-radius: var(--border-radius); margin-bottom: 1rem;">
          <option value="pending_confirmation" ${
            order.status === "pending_confirmation" ? "selected" : ""
    ...
    const content = `
      <div>
        ...
        <div style="background: var(--bg-secondary); padding: 1rem; border-radius: var(--border-radius); margin-bottom: 1rem;">
          <div><strong>Cliente:</strong> ${order.customer.firstName} ${
      order.customer.lastName
    }</div>
