export class AdminProductsPage {
        ...
        <div class="admin-content">
          ...
          <div style="background: white; border-radius: var(--border-radius); box-shadow: var(--shadow-sm); overflow: hidden;">
            <div style="padding: 1.5rem; border-bottom: 1px solid var(--border-color); display: flex; justify-content: space-between; align-items: center;">
              <h2 style="margin: 0;">Catálogo de Productos (${this.filteredProducts.length})</h2>
              <div style="display: flex; gap: 1rem;">
            ...
            ` : `
              <div class="text-center" style="padding: 3rem;">
                ...
                <p style="color: var(--text-secondary); margin-bottom: 2rem;">
                  ${this.currentCategory === 'all' ? 'Aún no hay productos en el catálogo.' : 'No hay productos que coincidan con los filtros aplicados.'}
                </p>
          ...
          <div style="border: 1px solid var(--border-color); border-radius: var(--border-radius); overflow: hidden; transition: transform 0.2s; cursor: pointer;" 
            ...
            <div style="padding: 1rem;">
              ...
              <div style="display: flex; gap: 0.5rem;">
                <button class="btn btn-sm btn-outline" onclick="this.viewProduct('${product.id}')" style="flex: 1;">👁️ Ver</button>
                <button class="btn btn-sm btn-primary" onclick="this.editProduct('${product.id}')" style="flex: 1;">✏️ Editar</button>
                <button class="btn btn-sm btn-error" onclick="this.deleteProduct('${product.id}')">🗑️</button>
              </div>
            </div>
          </div>
        `).join('')}
              ...
              <tr style="border-bottom: 1px solid var(--border-color);">
                <td style="padding: 1rem;">
                  <div style="display: flex; align-items: center; gap: 1rem;">
                    ...
                    <div>
                      ...
                      <div style="color: var(--text-secondary); font-size: 0.875rem;">${product.description.substring(0, 50)}...</div>
                    </div>
                  </div>
                </td>
                <td style="padding: 1rem;">
                  <span class="status-badge status-info">${this.getCategoryName(product.category)}</span>
                </td>
                <td style="padding: 1rem;">
                  <div style="font-weight: 600; color: var(--primary-color);">$${product.basePrice.toFixed(2)}/m²</div>
                </td>
                <td style="padding: 1rem;">
                  <div style="font-weight: 600;">${product.thickness}mm</div>
                </td>
                ...
                <td style="padding: 1rem; text-align: center;">
                  <div style="display: flex; gap: 0.5rem; justify-content: center;">
                    <button class="btn btn-sm btn-outline" onclick="this.viewProduct('${product.id}')">👁️</button>
                    <button class="btn btn-sm btn-primary" onclick="this.editProduct('${product.id}')">✏️</button>
                    <button class="btn btn-sm btn-error" onclick="this.deleteProduct('${product.id}')">🗑️</button>
                  </div>
                </td>
              </tr>
  ...
  productsToCSV() {
    const headers = ['ID', 'Nombre', 'Descripción', 'Categoría', 'Precio Base', 'Espesor', 'Variantes']
      ...
      <div style="max-height: 70vh; overflow-y: auto;">
        ...
        <div style="margin-bottom: 2rem;">
          ...
          <div style="background: var(--bg-secondary); padding: 1rem; border-radius: var(--border-radius); margin-top: 1rem;">
            ...
            <div><strong>Categoría:</strong> ${this.getCategoryName(product.category)}</div>
            <div><strong>Precio Base:</strong> $${product.basePrice.toFixed(2)}/m²</div>
            ...
            <div style="margin-top: 1rem;">
              ${product.variants.map(variant => `
                <div style="display: flex; justify-content: space-between; align-items: center; padding: 1rem; border: 1px solid var(--border-color); border-radius: var(--border-radius); margin-bottom: 0.5rem;">
                  <div>
                    <div style="font-weight: 600;">${variant.width}cm × ${variant.length}cm</div>
                    <div style="color: var(--text-secondary); font-size: 0.875rem;">Dimensiones estándar</div>
                  </div>
                  <div style="text-align: right;">
                    <div style="font-weight: 600; color: var(--primary-color);">$${variant.price.toFixed(2)}</div>
    ...
    const content = `
      <div>
        ...
        <div style="background: var(--bg-secondary); padding: 1rem; border-radius: var(--border-radius); margin-bottom: 1rem;">
          ...
          <div><strong>Categoría:</strong> ${this.getCategoryName(product.category)}</div>
          <div><strong>Precio:</strong> $${product.basePrice.toFixed(2)}/m²</div>
        </div>
